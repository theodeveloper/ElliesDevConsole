...
if($companyName =="" || $companyName!="none"){
    $companyName = $customer->CompanyName;
}else{
...
if($logo !="") {
    $image_file = $logo;
    $pdf->Image($image_file, 10, 85, 200, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
}else{
...
while($row = mysqli_fetch_assoc($sqlres)) {  
    ...
    if( $replaceFitting == "") $replaceFitting = "None";
    $Hours_Per_Day = 0;
    if(!empty($quoteitem->InputValues["Hours_Per_Day"])){
        $Hours_Per_Day = $quoteitem->InputValues["Hours_Per_Day"];
        if($Hours_Per_Day < 0)$Hours_Per_Day = 0;
    }
    if(!in_array($quoteitem->ProductCode, $arrAdditonal)){      
        $records  .= str_replace("^"," - ",$quoteitem->Room).",".$quoteitem->OldProductName.",".$quoteitem->Qty.",".$quoteitem->ItemDescription.",".$quoteitem->NewQty.",". $Hours_Per_Day.",".$replaceFitting.";";
    }else{
        $recordsAdditonal  .= str_replace("^"," - ",$quoteitem->Room).",".$quoteitem->OldProductName.",".$quoteitem->Qty.",".$quoteitem->ItemDescription.",".$quoteitem->NewQty.",". $Hours_Per_Day.",".$replaceFitting.";";     
    } 
    $totaliItemPrice += $quoteitem->NewQty * $quoteitem->TempPrice; 
    $itemcount += $quoteitem->NewQty; 
}
$totaliItemPrice = number_format($totaliItemPrice,2,".","");

if($records !== "" || $recordsAdditonal !== ""){
    $pdf->SetPrintHeader(true);
    // add a page
    $pdf->AddPage();
    ...
    $table = '
      <table border = "1" cellpadding = "2">
        <tr>
            ...
            <th bgcolor ="#8CC53E" width="25%" align="center">Existing Technology</th>
            <th bgcolor ="#8CC53E" width="10%" align="center">Quantity</th>
...
//Floor Plans
$sql = "SELECT *";
$sql.= " FROM `quote_floor_plans`";
$sql.= " WHERE `file_group`='PDF' AND `quote_id` = '".mysqli_real_escape_string($GLOBALS["link"],$quoteid)."' ORDER BY `id`";
$sqlres = mysqli_query($GLOBALS["link"],$sql);
$arrFloor = array();
while($row = mysqli_fetch_assoc($sqlres)) {
    $arrFloor[] =  $row['floor_plan'];
}
$numfloors =count($arrFloor);
//Replacement Products
$numProducts =count($arrProducts);
$path =dirname($_SERVER["SCRIPT_FILENAME"]);

if($numfloors > 0 && $numProducts > 0){
    $path =dirname($_SERVER["SCRIPT_FILENAME"]);
    error_reporting(E_ALL);
    ...
    {
        if(file_exists($path."/floorplans/". $arrFloor[$x])){
            $file = trim($arrFloor[$x]);
            $files[] = $path."/floorplans/".$file;
        }
    }
    ...
    //$fdpf->Output(__DIR__ .'/emailedquotes/Ellies Renewable Energy_'.$quoteid.'.pdf', 'F');
    $fdpf->Output('Ellies Renewable Energy_'.$quoteid.'.pdf', 'I');
}else{
